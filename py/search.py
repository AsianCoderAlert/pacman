...
def breadthFirstSearch(problem):
	...
	if problem.isGoalState(state) == True:
		...
		# next_state is something like (4, 2) (coordinates)
		# action is something like WEST
		# cost is not used for breadth-first search
		if not next_state in closedset:
                    # parent -> child, action
                    parents[next_state] = (state, action)
                    openset.insert(0, next_state)
                    closedset.append(next_state)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    heappush(openset, (start, heuristic(start, problem)))
    parents = {}
    while len(openset):
        state = heappop(openset)[0]
        closedset.append(state)
        if problem.isGoalState(state) == True:
	    print "Found goal!"
	    # retrieve series of steps that brought us here (use the parents map)
		...
		if not next_state in closedset:
                    # parent -> child, action
                    dist = heuristic(next_state, problem)
                    heappush(openset, (next_state, cost + dist))
                    parents[next_state] = (state, action)
                    closedset.append(next_state)
